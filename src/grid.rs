mod grid {
    ...
    impl<T: Eq> Grid<T> {
            ...
            match self.cubes.get(row * self.columns + col) {
                ...
                Some(candidate) => {
                    if candidate.row == row && candidate.col == col {
                        Some(candidate)
                    } else { // referred to incoherently with a non-extant column or row
                        None
                    }
                }
            }
        ...
        fn get_adjacent_to(&self, row: isize, col: isize) -> impl IntoIterator<Item=&Cube<T>> {
            ...
            for maybe_neighbor in &[up, up_right, right, down_right, down, down_left, left, up_left] {
    ...
    mod tests {
        ...
        fn get_adjacent() {
            ...
            assert!(by_top_corner.contains(&Cube { row: 0, col: 1, value: 'b' }));
            assert!(by_top_corner.contains(&Cube { row: 1, col: 0, value: 'e' }));
            assert!(by_top_corner.contains(&Cube { row: 1, col: 1, value: 'f' }));
            assert_eq!(by_top_corner.len(), 3);

            let by_bottom_corner: HashSet<&Cube<char>> = g.get_adjacent_to(3, 3).into_iter().collect();
            assert!(by_bottom_corner.contains(&Cube { row: 3, col: 2, value: 'o' }));
            assert!(by_bottom_corner.contains(&Cube { row: 2, col: 2, value: 'k' }));
            assert!(by_bottom_corner.contains(&Cube { row: 2, col: 3, value: 'l' }));
            assert_eq!(by_bottom_corner.len(), 3);

            let by_h: HashSet<&Cube<char>> = g.get_adjacent_to(1, 3).into_iter().collect();
            assert!(by_h.contains(&Cube { row: 0, col: 2, value: 'c' }));
            assert!(by_h.contains(&Cube { row: 0, col: 3, value: 'd' }));
            assert!(by_h.contains(&Cube { row: 1, col: 2, value: 'g' }));
            assert!(by_h.contains(&Cube { row: 2, col: 2, value: 'k' }));
            assert!(by_h.contains(&Cube { row: 2, col: 3, value: 'l' }));
            assert_eq!(by_h.len(), 5);

            let by_f: HashSet<&Cube<char>> = g.get_adjacent_to(1, 1).into_iter().collect();
            assert!(by_f.contains(&Cube { row: 0, col: 0, value: 'a' }));
            assert!(by_f.contains(&Cube { row: 0, col: 1, value: 'b' }));
            assert!(by_f.contains(&Cube { row: 0, col: 2, value: 'c' }));
            assert!(by_f.contains(&Cube { row: 1, col: 0, value: 'e' }));
            assert!(by_f.contains(&Cube { row: 1, col: 2, value: 'g' }));
            assert!(by_f.contains(&Cube { row: 2, col: 0, value: 'i' }));
            assert!(by_f.contains(&Cube { row: 2, col: 1, value: 'j' }));
            assert!(by_f.contains(&Cube { row: 2, col: 2, value: 'k' }));
